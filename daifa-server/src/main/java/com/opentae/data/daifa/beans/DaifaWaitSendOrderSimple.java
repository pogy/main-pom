...
public class DaifaWaitSendOrderSimple implements Serializable{
    ...
    @Column("daifa_wait_send_order.df_order_id")
    private String childOrderId;//子单id
    @Column("daifa_wait_send_order.pic_path")
    private String imgSrc;//商品图片
    @Column("daifa_wait_send_order.title")
    private String title;//商品标题
    @Column("daifa_wait_send_order.prop_str")
    private String goodsProperty;//商品属性 颜色 尺码
    @Column("daifa_wait_send_order.goods_code")
    private String goodsNo;//商品货号
    @Column("daifa_wait_send_order.single_price")
    private String piPrice;//商品批发价
    @Column("daifa_wait_send_order.single_pi_price")
    private String payPrice;//商品实际支付价
    @Column("daifa_wait_send_order.goods_num")
    private String num;//商品数量
    @Column("daifa_wait_send_order.single_services_fee")
    private String childServersFee;//子单服务费用
    @Column("daifa_wait_send_order.order_remark")
    private String childRemark;//子弹备注
    @Column("daifa_wait_send_order.allocat_status")
    private Integer allotState;//分配状态 1待分配2已分配
    @Column("daifa_wait_send_order.take_goods_status")
    private Integer takeGoodsState;//拿货状态 0拿货中 1已拿到 2缺货
    @Column("daifa_wait_send_order.have_goods_date")
    private String haveGoodsTime;//有货时间
    @Column("daifa_wait_send_order.delist_is")
    private Boolean noSaleIs;//是否下架true不卖了下架 false还在卖 注：该处为是否标记下架
    @Column("daifa_wait_send_order.refund_status")
    private Integer refundStatus;
    @Column("daifa_wait_send_order.return_goods_status")
    private Integer returnGoodsStatus;//退货状态@1等待退货2已退货3退货失败
    @Column("daifa_wait_send_order.change_status")
    private Integer changeStatus;//换货状态'1申请换货2已换货3换货失败发回4换货失败不发回'
    @Column("daifa_wait_send_order.store_goods_code")
