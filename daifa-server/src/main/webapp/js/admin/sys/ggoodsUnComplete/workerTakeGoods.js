...
/*====== dfgl/orderHasAllocation#printTypeTabs.js:0 ======*/
$(function(){
    $(document).on('click', '.printTypeTabs li', function(){
        var printType = $(this).attr('data-type');
        if(printType != '0'){
            $('#stateType').val(printType);
        }else{
            $('#stateType').val('');
        }
...
/*====== dfgl/page#searchKeydown.js:0 ======*/
$(function(){
    $('.orderSearchBox input').keydown(function(e){
        if(e.keyCode == 13){
            if($('.orderSearchBox [jbtn=searchBtn]')[0]){
                $('.orderSearchBox [jbtn=searchBtn]').click();
            }
        }
    });
});
...
/*====== dfgl/orderHasAllocation#orderSearch.js:1 ======*/
$(function(){
    $(document).on('click', '[jbtn=searchBtn]', function(){
        var data = {
            page: 1,
            startTime: $('.orderSearchBox [name=startTime]').val(),
            searchWorkerId:  $('.orderSearchBox [name=searchWorkerId]').val()
        };
        $$.search(data);
    });
});
...
/*====== global/form#checkbox.js:0 ======*/
$$('checkbox', function(){

    /*禁止label默认事件和冒泡，设置为手动触发设置input的值，因为safari阻止选中存在bug*/
    $(document).on('click', '.fmCheckbox', function(e){
        e.stopPropagation();
        e.preventDefault();
        $(this).find('input[type=checkbox]').trigger('setValue', !$(this).hasClass('checked'));
    });

    $(document).on('change', '.fmCheckbox input[type="checkbox"]', function(){

        var checked = $(this).prop('checked');
        // if(/msie\s+8/i.test(navigator.userAgent)){ //修正ie8不支持:checked
        if(checked){
        ...
        var $child = $([
            '.fmCheckbox input[data-selectParent="'+ name +'"]',
            '.fmCheckbox input[data-statusParent="'+ name +'"]'
        ].join(',')).each(function(){
            var childChecked = $(this).prop('checked');
            if(checked != childChecked){
                $(this).trigger('setValue', checked);
            }
        });

        setParent.call(this); //父级联动，父级分2种类型，分别有不同的实现逻辑

    });

    $(document).on('setValue', '.fmCheckbox input[type="checkbox"]', function(evt, value){
        var checked = $(this).prop('checked');
        if(checked != value){
    ...
    $(function(){
        $('.fmCheckbox input[type="checkbox"]:checked').change();
    });
    ...
    function setParent(){
        ...
        var selectParentName = $(this).attr('data-selectParent');
        var statusParentName = $(this).attr('data-statusParent');

        if(selectParentName){ //拥有全选形式的父级，比如全选按钮（理论存在）
            var $selectParent = $('.fmCheckbox input[name="'+ selectParentName +'"]');
            if($selectParent[0]){ //事实存在
                //子有一项未选中，则不选中，否则选中
                var $child = $('.fmCheckbox input[data-selectParent="'+ selectParentName +'"]');
                if($child.length){
                    var newChecked = true; //默认选中
                    $child.each(function(){
                    ...
                    $selectParent.each(function(){ //改变状态
                        var oldChecked = $(this).prop('checked');
        ...
        }else if(statusParentName){ //拥有表示状态的父级，比如订单与子单的关系
            var $statusParent = $('.fmCheckbox input[name="'+ statusParentName +'"]');
            if($statusParent[0]){ //事实存在
                //子有一项选中，则选中，否则不选中
                var $child = $('.fmCheckbox input[data-statusParent="'+ statusParentName +'"]');
                if($child.length){
                    var newChecked = false; //默认不选中
                    $child.each(function(){
                        if($(this).prop('checked')){
                            newChecked = true;
                            return false;
                    ...
                    $statusParent.each(function(){ //改变状态
                        var oldChecked = $(this).prop('checked');
                        if(oldChecked != newChecked){
                            $(this).prop('checked', newChecked);
                            $(this).parent().toggleClass('checked');
...
/*====== dfgl/orderHasAllocation#childOrderItem.js:0 ======*/
$(function(){
    $(document).on('click', '[jbtn=isGetGoods]', function(){
        var type = $(this).attr('data-type');
        var takeGoodsId = $(this).parents('.childOrderItem').attr('data-takeGoodsId');
        var comfirmText = type == "1" ? '已拿到?' : '未拿到?';
        $$.confirm('确认'+comfirmText, function(){
            $$.post('setIsGetGoodsJson.json', function(resp){
                if(resp.err){
            ...
            },{type: type, takeGoodsId: takeGoodsId},{});
        });
...
/*====== dfgl/page#setHaveGoodsTime.js:0 ======*/
$(function(){
...
/*====== dfgl/orderHasAllocation#orderCon.js:0 ======*/
$(function(){
...
/*====== dfgl/page#doCaiNiaoPrint.js:0 ======*/
//http://cloudprint.cainiao.com/print/login.vm?__output_charset=gbk&redirectURL=https%3A%2F%2Flogin.taobao.com%2Fmember%2Flogin.jhtml%3Fredirect_url%3Dhttp%3A%2F%2Fcloudprint.cainiao.com%2Fprint%2FchooseIsv.htm%3FafterChoosenIsv%3Dhttp%3A%2F%2Fcloudprint.cainiao.com%2Fcloudprint%2Ftemplates.htm  菜鸟打印模板

$$('doCaiNiaoPrint', function(){
    var PID = 0;
    var webSocket; //备注：webSocket 是全局对象，不要每次发送请求丢去创建一个，做到webSocket对象重用，和打印组件保持长连接。
    var tr;
    try{
        doConnect();
    }catch(e){
    ...
    doPrint.onPrint = function(){};
    doPrint.preview = true; //是否预览模式
    doPrint.templateURL = $stylepath + 'dfgl/print_tpl/common_post.js'; //模板地址
    return doPrint;
    ...
    function doConnect(){
        ...
        webSocket.onopen = function(event){
            ...
            console.log('打印接口连接成功...');

            // 监听消息
            webSocket.onmessage = function(event){
                var data = JSON.parse(event.data);
                console.info('打印接口响应数据', data);
                if(data.cmd == 'getPrinters'){
                    allPrints = data.printers;
                }
                if(data.cmd == 'print'){
                    doPrint.onPrint(data);
                    //$('#viewPic').attr('src', data.previewImage[0]);
                }
    ...
    function doPrint(data){
        var request  = makeRequest("print", {
            task: {
                taskID: ''+ (+new Date) + ++PID,
                preview: doPrint.preview,  //如果为true，则是预览模式，为false，则直接打印
                previewType: "image",
                notifyMode:"oneByOne",//打印结果通知模式，是逐个document通知还是批量一次通知最终打印结果
                ...
                documents: [
                    {
                        ...
                        contents: [
                            {
                                templateURL: data.templateURL || doPrint.templateURL,
                                data: data
                            }
                        ]
                    }
...
/*====== dfgl/orderHasAllocation#orderCon.js:1 ======*/
$(function(){

    $$.doCaiNiaoPrint.preview = false;
    $$.doCaiNiaoPrint.onPrint = function(data){
        if(data.previewImage){
            window.open(data.previewImage[0]);
        }
    };
    $$.doCaiNiaoPrint.templateURL = $stylepath + 'dfgl/print_tpl/common_lable.js?v5'; //模板地址;
    //$$.doCaiNiaoPrint.templateURL = 'http://192.168.1.66:666/style/dfgl/print_tpl/common_lable.js'; //模板地址;
    //$$.doCaiNiaoPrint.templateURL = 'http://style.571xz.com/v2/dfgl/print_tpl/common_lable.js?444'; //模板地址;

    $('.printGoodsTab').click(function(){
        var type = $(this).attr('data-type');

        if(type == "2"){
            var ids = [];
            $('.childOrderItem').each(function(){
                if($(this).find('.fmCheckbox').hasClass('checked')){
                    ids.push($(this).attr('data-takeGoodsId'));
                }
            });
            if(ids.length < 1){
                return $$.windowOperate.errorTip('请选择子单');
            }
            ids = ids.join(',');
        }

        $$.post('printGoodsTabJson.json',function(resp){
            if(resp.err){
                $$.windowOperate.errorTip(resp.err);
            }else{
                $.each(resp.tabList, function(i, printInfo){
                    if(printInfo.orderSort == '1-1' || printInfo.orderSort == ''){
                        printInfo.orderSort = "";
                        printInfo.specialStr = "";
                    }
                    printInfo.printer = 'tabPrint'; //标签打印机
                    $$.doCaiNiaoPrint(printInfo);
                    //$$.printLabelPage(printInfo);
                });
            }
        },{ids: ids, type: type}, {});
