...
$(function(){
    var objLay = "";  //0:宝贝图片 1：手机端描述插入图片 2：颜色选择图片 3：商品描述图
    ...
    $("#smallTitleID").keyup(function(){
        var txtCount = 20;
        var obj = $('#smallTitleID');
        var errID = $("#err_nav_smallTitle");
        getBt(obj,txtCount,errID);
    });
    ...
    var buyNowPrice = $("#pPrice").val(); //团购价
    var buyOnePrice = (parseFloat($("#pPrice").val()) * 2).toFixed(2);
    $(".sku-color li").find(".J_Checkbox").on("click",function(){
        ...
        if(inputC.is(":checked")){
            ...
            $(".J_SKUImgTable tbody tr").each(function(){
                var imgId = $(this).attr("id").substr(9); //id="J_MapImg_1627207-28327"
                if(propId==imgId){
                    $(this).show();
                }
            });
            ...
            var totalColorNum = $('.sku-color .edit').length;
            var addHtml = "";
            var hasSize = $(".sku-group").hasClass("sku-size");
            var dataPropIdC = propId //1627207-3232483 对应颜色
            if(hasSize){  //存在尺码
                var sizeCount  = $(".sku-size").find(".edit").length;
                $(".sku-size").find(".edit").each(function(i,e){
                    var checkboxId = $(e).find(".J_Checkbox").attr("id").substr(5); //id="prop_20509_25054027"
                    var sizeVal = $(e).find(".editbox ").val();
                    var dataPropIdS = checkboxId //20509-25054027  对应尺码

                    if(i==0){
                        addHtml += '<tr class="J_MapRow" data-color="'+dataPropIdC+'">';
                        addHtml += '<td rowspan="'+sizeCount+'" class="tile">';
                        addHtml += '<span class="J_MapColor_'+dataPropIdC+'">'+labelE.html()+'</span></td>';
                        addHtml += '<td rowspan="1">';
                        addHtml += '<span class="J_MapSize_' +dataPropIdS+'">'+sizeVal+'</span></td>';
                        addHtml += '<td class="gprice">';
                        addHtml += '<input data-id="'+dataPropIdC+'_'+dataPropIdS+'" id="J_SkuField_gprice_'+dataPropIdC+'_'+dataPropIdS+'" class="J_MapPrice text" data-type="gprice" type="text" value="'+buyNowPrice+'"></td>';
                        addHtml += '<td class="price">';
                        addHtml += '<input data-id="'+dataPropIdC+'_'+dataPropIdS+'" id="J_SkuField_price_'+dataPropIdC+'_'+dataPropIdS+'" class="J_MapOnePrice text" data-type="price" type="text" value="'+buyOnePrice+'"></td>';
            ...
            //颜色隐藏
            $(".J_SKUImgTable tbody tr").each(function(){
                var imgId = $(this).attr("id").substr(9);
                if(propId==imgId){
                    $(this).hide();
                }
            });
    ...
    $(".sku-size li").find(".J_Checkbox").on("click",function(){
        ...
        var labelE = skuLi.find(".labelname");
        var inputE = skuLi.find(".editbox");
        var propId = inputC.attr("id").substr(5); //id="prop_20509_25054027"
        ...
        if(inputC.is(":checked")){
            ...
            if(tbodyStr==0){
                for(i=0;i<colorCount;i++){
                    ...
                    addHtml += '<tr class="J_MapRow" data-color="'+dataPropIdC+'">';
                    addHtml += '<td rowspan="1" class="tile"><span class="J_MapColor_'+dataPropIdC+'">'+firstTtml+'</span></td>';
                    addHtml += '<td rowspan="1">';
                    addHtml += '<span class="J_MapSize_'+propId+'">'+checkVal+'</span></td>';
                    addHtml += '<td class="gprice">';
                    addHtml += '<input data-id="'+dataPropIdC+'_'+propId+'" id="J_SkuField_gprice_'+dataPropIdC+'_'+propId+'" class="J_MapPrice text" data-type="gprice" type="text" value="'+buyNowPrice+'"></td>';
                    addHtml += '<td class="price">';
                    addHtml += '<input data-id="'+dataPropIdC+'_'+propId+'" id="J_SkuField_price_'+dataPropIdC+'_'+propId+'" class="J_MapOnePrice text" data-type="price" type="text" value="'+buyOnePrice+'"></td>';
                    addHtml += '<td class="quantity">';
                    addHtml += '<input maxlength="9" data-id="'+dataPropIdC+'_'+propId+'" id="J_SkuField_quantity_'+dataPropIdC+'_'+propId+'" class="J_MapQuantity text" data-type="quantity" type="text" value=""></td>';
                    addHtml += '<td class="tsc">';
                    addHtml += '<input data-id="'+dataPropIdC+'_'+propId+'" id="J_SkuField_tsc_'+dataPropIdC+'_'+propId+'" class="J_MapProductid text" data-type="tsc" type="text" value=""></td>';
                    addHtml += '</tr>';
                    $(".sku-loading tbody").append(addHtml);
                }
            }else{
                ...
                if(curPre==0){
                    for(i=0;i<colorCount;i++){
                        ...
                        addHtml += '<tr class="J_MapRow" data-color="'+dataPropIdC+'">';
                        addHtml += '<td rowspan="'+(sizeCount+1)+'" class="tile"><span class="J_MapColor_'+dataPropIdC+'">'+firstTtml+'</span></td>';
                        addHtml += '<td rowspan="1">';
                        addHtml += '<span class="J_MapSize_'+propId+'">'+checkVal+'</span></td>';
                        addHtml += '<td class="gprice">';
                        addHtml += '<input data-id="'+dataPropIdC+'_'+propId+'" id="J_SkuField_gprice_'+dataPropIdC+'_'+propId+'" class="J_MapPrice text" data-type="gprice" type="text" value="'+buyNowPrice+'"></td>';
            ...
            $(".J_MapRow .tile").attr("rowspan",sizeCount-1);
            // 判断是否删除第一个
            var mapId = "J_MapSize_" + propId; //对应的尺码
    ...
    //计算商品数量
    function resetGoodsAllNum() {
        var allNum = 0;
        $('.J_MapQuantity').each(function(){
            if($(this).val()){
                allNum = allNum+parseFloat($(this).val());
            }
        });
        $('#quantityId').val(allNum);
    }
    ...
    // 添加图片
    $('.long-img .addDimg').click(function(){

    });

    // 删除图片
    $(document).on("click", ".operate .del",function(){
        if($(this).parents('.long-img')[0]){
	...
	$(".multimage-wrap").delegate(".toleft","click",function(){
		...
		var direction = 0;
		var $parents = $(this).parents('.multimage-wrap');
		moveImg(direction,index,$parents);
    });
    //右移动图片
	...
	function moveImg(direction,index,$parents){
        var $sibIndex;
		if(direction == 0 && index != 0){
            $sibIndex = $parents.find(".image-list li[data-index="+index+"]").prev('li');
		}
		if(direction == 1){
            $sibIndex = $parents.find(".image-list li[data-index="+index+"]").next('li');
		}

		if($sibIndex[0]){

			var $curIndex = $parents.find(".image-list li[data-index="+(index)+"]");
	...
	//渲染描述图数量
    function renderDescImgNum() {
	    var imgNum = $('.long-img li').length;
        showHideAddBtn(imgNum);
        $('.descImgTip').html('当前商品共有 '+ imgNum + ' 张图片');
    }

    //描述图添加按钮展示隐藏
    function showHideAddBtn(imgNum) {
        if(imgNum >= 20){
            $('.addDimg').hide()
        }else{
            $('.addDimg').show()
    ...
    //描述图添加按钮操作
    $('.addDimg').click(function(){
        objLay = 3;
        descImgNum = descImgNum + 1;
        imgLay=layer.open({
            type: 1,
            title: '在线编辑图片',
            closeBtn: 1,
    ...
    $("#J_ListTable").delegate("li","click",function(){
        ...
        if(objLay==0){   //插入宝贝图片
            ...
            imgHtml += '<li data-index="'+ descImgNum +'" class="has-media">\n' +
                        ...
                        '<div class="preview ">\n' +
                            '<div class="img">\n' +
                                '<a href="'+ imgUrl +'" target="_blank"> <img src="'+ imgUrl +'_400x400.jpg"></a>\n' +
                            '</div>\n' +
                        '</div>\n' +
                    '</li>';

            $('.long-img ul .addDimg').before(imgHtml);
            renderDescImgNum();
        }
