...
<html>
  <head>
    ...
    <link href="http://style.571xz.com/gys4/css/fitmentStorefitmentStore.css?t=" rel="stylesheet">
...
<div class="wrapper layout">
    ...
    <div class="sidebar shadow-box">
        ...
        <ul>
            ...
            <li><a class="selected"  href="${main_host!}seller/fitmentStorefitmentStore.htm?tab=1">店铺装修</a></li> 
    ...
    <div class="content shadow-box">
        <ul class="tabbox clearfix">
            <li class="selected" data-type="edit-logo"><span>LOGO更换</span></li>
            <li data-type="edit-banner"><i class="fl"></i><span>banner管理</span><i class="fr"></i></li>
            <li data-type="edit-content"><span>编辑自定义内容</span></li>
        </ul>
        <div class="contentbox edit-logo">
            <div class="editbox">
                <div class="edit-logo-box">
                    <div class="lf-box">
                        <div class="logopic"></div>
                        <label><input type="checkbox" id="is_open_logo">显示LOGO</label>
                    </div>
                    <div class="rt-box">
                        <button class="view-file-button">浏览...</button>
                        <input class="view-file-input logo_file" accept=".jpg,.jpeg,.png,.gif,.bmp" type="file" name="Filedata">
                        <button class="file-delete" id="rmvlogo">删除</button>
                        <p class="tips">*注：图片推荐宽度100px，高度100px</p>
<#assign text>{"text":'保存设置',"id":'savelogo'}</#assign>
<#assign $it=text?eval />
                                
                            <button class="button-base button-1" id="savelogo">保存设置</button>
                        
                        
                    </div>
                </div>
                <div class="edit-banner-box">
                    <ul class="bannerbox">
                        <li class="clearfix" p="1">
                            <div class="index">1、</div>
                            <div class="picbox"><img src="http://style.571xz.com/gys/css/img/zxbanner.png" /></div>
                            <div class="uploadbox">
                                <button class="view-file-button">浏览...</button>
                                <input class="view-file-input" accept=".jpg,.jpeg,.png,.gif,.bmp" name="Filedata" type="file">
                                <button class="file-delete">删除</button>
                            </div>
                        </li>
                    </ul>
                    <div class="infobox">
                        <span class="banner-add"><i class="iconfont">&#xe6c8;</i>添加</span>
                        <p>*注：图片推荐宽度1200px，高度360px</p>
                        <label><input type="checkbox" id="banner_is_open" data-alt-no="Banner">显示幻灯片</label>
<#assign text>{"text":'保存设置', "id":'save_banner'}</#assign>
<#assign $it=text?eval />
                                
                            <button class="button-base button-1" id="save_banner">保存设置</button>
                ...
                <script type="ccc">
                    ...
                    $(document).on("click",".file-delete",function(){
                        //var li_ = del_img_com(this);
                        $(".banner-add").show();
                        var li_ = $(this).parents("li").eq(0);
                        var ul_ = li_.parent(".bannerbox");
                        li_.remove();
                    ...
                    //--背景的前标的重置
                    function Reset_banner_ul(j_upList){
                            //-- id 重置
                            j_upList.find("li").each(function(index){
                    ...
                    var ul_ = $(this).parents(".edit-banner-box").find(".bannerbox");
                    var li_length = ul_.find("li").length;
                        if(li_length>=MAX_Banner){
                        
                        return;
                        }else{
                            li_length+=1;
                            ul_.append('<li class="clearfix"><div class="index">'+(li_length)+'、</div><div class="picbox" data-index="'+li_length+'"></div><div class="uploadbox"><input  class="view-file-input-banner" accept=".jpg,.jpeg,.png,.gif,.bmp" type="file" name="file" onchange="imageUpload(this,&quot;banner&quot;)"><button class="view-file-button">浏览...</button><input class="view-file-input" accept=".jpg,.jpeg,.png,.gif,.bmp" name="file" type="file"><button class="file-delete">删除</button></div></li>');
                    ...
                    //--没图片的时候是不能点的
                    $('#bannerSub_1').click(function(){
                        if(!$('.bannerbox').find(".picbox img")[0] && $('#is_open').prop('checked')){
                            alert("没有上传图片，无法保存显示幻灯片！");
                            return;
                        }
                        ImgSubmit();
                ...
                <div class="edit-content-box">
                    <label><input type="checkbox" id="is_open_desc">显示自定义内容</label>
                    <textarea id="ks-editor-textarea"></textarea>
<#assign text>{"text":'保存编辑',"id":'save_desc'}</#assign>
<#assign $it=text?eval />
                        
                        <button class="button-base button-1" id="save_desc">保存编辑</button>
                    ...
                    <script src2="http://www.571xz.com/js/memberStore/deonkey.js"></script>
                    
                </div>
            </div>
            <div class="viewbox">
                <i class="i-tips"></i>
                <img width="494" height="316" src="http://static.571xz.com:86/images/member/ylPic.jpg">
            </div>
        </div>
    </div>
</div>
...
<script src="http://style.571xz.com/gys4/js/fitmentStorefitmentStore.js?t="></script>
