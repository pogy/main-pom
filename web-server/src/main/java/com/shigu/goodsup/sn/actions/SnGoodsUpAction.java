...
public class SnGoodsUpAction {
    ...
    public String publish(Long itemId, HttpServletRequest request, HttpSession session, Map<String, Object> map, String cid) {
        try {
            ...
            if (cid == null) {
                if (categoryQueries.size() != 1) {
                    List<SnCatVo> list = new ArrayList<>();
                    for (CategoryQueryResponse.CategoryQuery c : categoryQueries) {
                        SnCatVo vo = new SnCatVo();
                        vo.setCid(c.getCategoryCode());
                        vo.setCname(c.getDescPath());
                        list.add(vo);
                    }
                    SnQueryVo snQueryVo = new SnQueryVo(categoryQueries.get(0).getCategoryCode(), itemId);
                    map.put("cates", list);
                    map.put("query", snQueryVo);
                    return "suning/changeGoodsCate";
                }
                categoryCode = categoryQueries.get(0).getCategoryCode();
            } else {
                categoryCode = cid;
            }
