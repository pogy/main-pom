...
public class GoodsupRecordSimpleService {
    ...
    //    //{条件搜索代码段  boolQuery....   }
    //    //todo:分页，字段名要由前端确认
    //    shigugoodsup.setQuery(boolQuery).setFrom((bo.getNumber()-1)*bo.getSize()).setSize(bo.getSize()).addSort("daiTime", SortOrder.DESC);
    //    SearchResponse searchResponse = shigugoodsup.execute().actionGet();
