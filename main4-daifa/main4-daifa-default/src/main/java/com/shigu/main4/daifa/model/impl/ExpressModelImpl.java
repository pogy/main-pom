...
public class ExpressModelImpl implements ExpressModel {
        ...
        if (dce != null) {
            ...
            if("YZPY".equals(express.getKdType())){
                ...
                if(reCityCodeBean==null){
                    throw new DaifaException("系统未找到该地址市级的对应信息,请联系管理员", DaifaException.DEBUG);
                }
                receiverZipCode=reCityCodeBean.getZipCode();
                if(send.getReceiverArea()!=null){
                    ZipCodeBean reTownCodeBean = daifaPostCustomerMapper
                            .selectPostCodeByTown(send.getReceiverArea(), reCityCodeBean.getAddressId());
                    if(reTownCodeBean!=null){
                        receiverZipCode=reTownCodeBean.getZipCode();
                    }
                }
                String sendZipCode;
                ZipCodeBean sendCityCodeBean=daifaPostCustomerMapper.selectPostCodeByCity(send.getSendCity());
                sendZipCode=sendCityCodeBean.getZipCode();
                ZipCodeBean sendTownCodeBean=daifaPostCustomerMapper.selectPostCodeByTown(send.getSendArea(),sendCityCodeBean.getAddressId());;
                if(sendTownCodeBean!=null){
                    sendZipCode=sendTownCodeBean.getZipCode();
                }
                send.setReceiverZipCode(receiverZipCode);
                send.setSendZipCode(sendZipCode);
            }
    ...
    private QueryPostCodeVO getPostCode(SendBean send, ExpressBean express) throws KdApiException {
        ...
        receiver.setPostCode(send.getReceiverZipCode());//收件人邮编
        ...
        sender.setPostCode(send.getSendZipCode());//发货人邮编
        ...
        try {
            ...
            params.put("RequestData", KdHttpUtil
                    .urlEncoder(requestData, "UTF-8"));//请求内容需进行URL(utf-8)编码。请求内容JSON格式，须和DataType一致。
            ...
            params.put("DataSign", KdHttpUtil
                    .urlEncoder(dataSign, "UTF-8"));//数据内容签名：把(请求内容(未编码)+AppKey)进行MD5加密，然后Base64编码，最后 进行URL(utf-8)编码
